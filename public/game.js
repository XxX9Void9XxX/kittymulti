...
canvas.addEventListener("click", e => {
  socket.emit("shoot", { x: e.offsetX + cameraX, y: e.offsetY + cameraY });
});
...
socket.on("state", state => { gameState = state; console.log("Mice:", state.mice); });
...
function draw() {
  ...
  if (me) { cameraX = Math.max(0, Math.min(world.width - canvas.width, me.x - canvas.width / 2)); cameraY = Math.max(0, Math.min(world.height - canvas.height, me.y - canvas.height / 2)); }

  ctx.fillStyle = "#5c94fc"; ctx.fillRect(0, 0, canvas.width, canvas.height);
  ctx.save(); ctx.translate(-cameraX, -cameraY);

  // Ground
  ctx.fillStyle = "#228B22"; ctx.fillRect(0, world.groundY, world.width, world.height);
  // Platforms
  ...
  mice.forEach(m => {
    ...
    ctx.fillStyle="lime"; ctx.fillRect(m.x, m.y-6,32*(m.hp/20),4);
  });

  // Players
  for (const id in players) {
    const p = players[id];
    ctx.save(); ctx.translate(p.x+24,p.y); ctx.scale(p.facingLeft?-1:1,1);
    if (playerReady) ctx.drawImage(playerSprite,-24,0,48,48);
    else { ctx.fillStyle="orange"; ctx.fillRect(-24,0,48,48);}
    ctx.restore();
    ctx.fillStyle="white"; ctx.font="12px Arial"; ctx.textAlign="center"; ctx.fillText(p.name,p.x+24,p.y-18);
    // Health
    ctx.fillStyle="red"; ctx.fillRect(p.x,p.y-12,48,6); ctx.fillStyle="lime"; ctx.fillRect(p.x,p.y-12,48*(p.hp/100),6);
  }
